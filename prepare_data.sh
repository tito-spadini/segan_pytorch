# Source: https://github.com/santi-pdp/segan/blob/master/prepare_data.sh
# Version: 2.0
...
if [ ! -d clean_trainset_wav_16k ]; then
    ...
    if [ ! -d clean_trainset_wav_16k ]; then
        ...
        ls *.wav | while read name; do
            sox $name -r 16k -b 16 --norm=-3 ../clean_trainset_wav_16k/$name
        done
        popd
    fi
fi
if [ ! -d noisy_trainset_wav_16k ]; then
    ...
    if [ ! -d noisy_trainset_wav_16k ]; then
        ...
        ls *.wav | while read name; do
            sox $name -r 16k -b 16 --norm=-3 ../noisy_trainset_wav_16k/$name
        done
        popd
    fi
fi
